...
const FireExtinguisherCamera = ({ onExit, onCapture }) => {
    ...
    const openCamera = async (facing = facingMode) => {
        console.log(`FireExtinguisherCamera: Attempting to open camera with facing mode: ${facing}...`);
        try {
            setIsLoading(true);
            setError(null);
            ...
            const newStream = await navigator.mediaDevices.getUserMedia({
                video: {
                    width: { ideal: 1280 },
                    height: { ideal: 720 },
                    facingMode: facing // 'user' for front camera, 'environment' for rear camera
                }
            });
            
            setStream(newStream);
                
                console.log("FireExtinguisherCamera: Camera stream obtained.", newStream);
                ...
                if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
                    ...
                    errorMessage += 'Requested camera mode not available. Trying alternative...';
                    // Try the opposite camera if the requested one fails
                    const fallbackMode = facing === 'environment' ? 'user' : 'environment';
                    console.log(`Trying fallback camera mode: ${fallbackMode}`);
                    setTimeout(() => openCamera(fallbackMode), 1000);
                    return;
                } else {
            ...
            <div className="relative">
                ...
                <div className="camera-preview-wrapper">
                    <div className="camera-preview-container">
                        ...
                        {/* Camera Controls - positioned within the styled preview */}
                        {!isLoading && !error && (
                            <>
                                ...
                                <div className="absolute top-4 right-4">
                                    <button
                                        ...
                                        className="bg-black/50 text-white p-3 rounded-full hover:bg-black/70 transition-all duration-300 transform hover:scale-105 shadow-lg"
