...
@app.get("/api/places/pye-barker")
async def get_pye_barker_locations(lat: float, lng: float, radius: int = 50000):
    """
    Find nearby Pye Barker Fire & Safety locations using Google Places API.
    """
    try:
        if not GOOGLE_PLACES_API_KEY:
            # Return mock Pye Barker data if no API key
            print("⚠️ No Google Places API key configured, returning mock Pye Barker data")
            return {
                "success": True,
                "locations": [
                    {
        ...
        # Search for Pye Barker locations specifically
        url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json"
        params = {
            'location': f"{lat},{lng}",
            'radius': radius,  # 50km radius for Pye Barker locations
            'keyword': 'Pye Barker Fire Safety',
            'type': 'establishment',
            'key': GOOGLE_PLACES_API_KEY
        }
        ...
        if response.status_code != 200:
            print(f"❌ Google Places API error for Pye Barker search: {response.status_code}")
            return {"success": True, "locations": []}  # Return empty instead of error
        
        data = response.json()
        
        if data.get('status') not in ['OK', 'ZERO_RESULTS']:
            print(f"❌ Google Places API status for Pye Barker: {data.get('status')}")
            return {"success": True, "locations": []}
        
        # Process Pye Barker locations
        locations = []
        for place in data.get('results', []):
            if 'pye barker' in place.get('name', '').lower():
                locations.append({
                    "name": place.get('name', 'Pye Barker Fire & Safety'),
                    "address": place.get('vicinity', 'Address not available'),
                    "phone": place.get('formatted_phone_number', 'Phone not available'),
                    "place_id": place.get('place_id'),
                    "rating": place.get('rating', 0),
                    "distance": "Distance calculating..."  # Could calculate actual distance if needed
                })
        ...
        print(f"✅ Found {len(locations)} Pye Barker locations")
